...
def app():
    ...
    if submit_button:
        ...
        avg_gradcam = getAttMap(norm_img, gradcam[0][1].numpy().astype(np.float32), blur=True)
        ...
        for _ in range(num_rows):
            with st.container():
                for col in st.columns(int(num_cols)):
                    ...
                    gradcam_todraw = getAttMap(norm_img, gradcam_img.numpy().astype(np.float32), blur=True)
